...
const main = async (current, cityName) => {
  ...
  let current_time_day = parseFloat(current_time.slice(7, -54));
  let current_year = parseFloat(current_time.slice(11, -48));
...
function weekday(day) {
  let today = day.slice(0, -61);
  let name;
  if (today.includes("Mon")) name = "Monday";
  if (today.includes("Tue")) name = "Tuesday";
  if (today.includes("Wed")) name = "Wednesday";
  if (today.includes("Thu")) name = "Thursday";
  if (today.includes("Fri")) name = "Friday";
  if (today.includes("Sat")) name = "Saturday";
  if (today.includes("Sun")) name = "Sunday";
  return name;
}
function day_for_forecast(date, dayCount) {
  let sliced_date = date.slice(4, 21);
  let month = month_maker(sliced_date);
  let day = parseInt(sliced_date.slice(5, -11)) + dayCount;
  let year = parseInt(sliced_date.slice(7, -6));
  let hours = parseInt(sliced_date.slice(12, -3));
  let minutes = parseInt(sliced_date.slice(15));
  let new_date = new Date(year, month - 1, day, hours, minutes);
  let what_day = new_date.getDay(new_date);
  let new_day = parseInt(new_date.toString().slice(7, 10));
  let new_month = month_maker(new_date.toString().slice(4, 7));
